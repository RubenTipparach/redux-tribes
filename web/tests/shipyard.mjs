...
async function checkLayout(page, label) {
  ...
  if (!pane.length) ok(`${label} [armour]: mode, palette, swatches, brush, surfaces and sliders all reachable`);
}
...
/**
 * The palette, the surfaces and the brush.
 *
 * WHICH palette is a dropdown of five; every surface has its own row and each
 * reaches the model; and the brush lays a colour on the cells a stroke crosses
 * and nowhere else.
 */
async function checkPaint(page, label) {
  await page.click('#dzTabArmour');
  await page.waitForTimeout(300);
  await checkCopy(page, label);
  const palette = await page.evaluate(() => {
    const el = document.getElementById('dzPaletteSel');
    if (!el) return null;
    return { tag: el.tagName, opts: [...el.options].map(o => o.value), value: el.value };
  });
  if (!palette) fail(`${label}: no palette dropdown on the armour tab`);
  else if (palette.tag !== 'SELECT')
    fail(`${label}: the palette control is a ${palette.tag}, not a dropdown`);
  else if (palette.opts.length < 5)
    fail(`${label}: the palette dropdown offers ${palette.opts.length} palettes, not five`);
  else {
    ...
    if (now.faction !== other)
      fail(`${label}: picked the ${other} palette and the design says ${now.faction}`);
    else if (now.hullTone === was)
      fail(`${label}: swapping to the ${other} palette left the plating 0x${was.toString(16)}`);
    else ok(`${label}: the palette is a dropdown of ${palette.opts.length}: `
  ...
  if (slotRows.length !== 8)
    fail(`${label}: ${slotRows.length} slot surface rows, not 8: ${named.join(', ')}`);
  else if (missing.length)
    fail(`${label}: no surface dropdown for ${missing.join(', ')}`);
  else if (slotRows.some(r => !r.chip))
    fail(`${label}: a slot surface row has no colour on it, so nothing says which slot it is`);
  else if (surf.some(r => r.opts < 2))
    fail(`${label}: a surface row has no finishes to choose from`);
  else ok(`${label}: ${surf.length} surface dropdowns: eight slots each beside its colour, `
  ...
  // drawn out of the subsystems' material. Read off the live materials,
  // because a finish that never reached one draws exactly like a finish
  // nobody picked.
  ...
  for (const m of set) fail(`${label}: ${m}`);
  await page.waitForTimeout(700);
  ...
  if (files.some(f => !f || f === 'none'))
    fail(`${label}: the yard draws no normal map on `
      + MACHINERY.filter((k, i) => !files[i] || files[i] === 'none').join(', '));
  else if (new Set(files).size !== 4)
    fail(`${label}: four different finishes were picked and the yard drew ${new Set(files).size}: `
      + `${files.join(', ')}`);
  else if (MACHINERY.some(k => !drawn[k].loaded))
    fail(`${label}: a yard surface has a normal map with no pixels: `
      + MACHINERY.filter(k => !drawn[k].loaded).join(', '));
  else ok(`${label}: each machinery dropdown reaches its own surface in the yard: ${files.join(', ')}`);

  // The BRUSH: a colour picked up, a stroke across the model, cells painted
  // and the hull colour untouched; the eraser takes them off; wipe empties
  // the list. Counted off the design, and the plating's own tone off the
  // model, because "one panel changed and the ship did not" is the whole
  // complaint the brush answers.
  const before = await page.evaluate(() => {
    const d = window.ftDebug.designer();
    return { paint: d.paint, tone: d.hullTone, tint: d.tint, yaw: d.cam.yaw };
  });
  const chips = await page.$$eval('#dzBrush .dzsw:not(.eraser)', b => b.length);
  if (chips !== 8) fail(`${label}: the brush offers ${chips} colours, not eight`);
  await page.click('#dzBrush .dzsw:nth-child(3)');
  await page.waitForTimeout(200);
  const armed = await page.evaluate(() => window.ftDebug.designer().brushSlot);
  if (armed !== 2) fail(`${label}: armed brush slot ${armed}, expected 2`);
  const box = await page.locator('#dzCanvas').boundingBox();
  const cx = box.x + box.width / 2, cy = box.y + box.height / 2;
  const stroke = async (dy = 0) => {
    await page.mouse.move(cx - 50, cy + dy);
  ...
  for (const dy of [0, 40, -40]) {
    ...
    if (after.tint >= 2) break;
  }
  if (after.tint < 2) fail(`${label}: a brush stroke across the hull painted ${after.tint} cell(s)`);
  else if (after.paint !== before.paint || after.tone !== before.tone)
    fail(`${label}: the brush moved the hull colour (0x${before.tone.toString(16)} to 0x${after.tone.toString(16)})`);
  else if (Math.abs(after.yaw - before.yaw) > 1e-6)
    fail(`${label}: the brush stroke turned the model`);
  else ok(`${label}: the brush paints ${after.tint} cells and the hull colour stays 0x${after.tone.toString(16)}`);
  await page.click('#dzBrushEraser');
  await page.waitForTimeout(200);
  for (const dy of [0, 40, -40]) await stroke(dy);
  const wiped = await page.evaluate(() => window.ftDebug.designer().tint);
  if (wiped >= after.tint) fail(`${label}: the brush eraser left ${wiped} of ${after.tint} painted cells`);
  else ok(`${label}: the brush eraser lifts them, ${after.tint} to ${wiped}`);
  if (wiped) {
    await page.click('#dzTintClear');
    await page.waitForTimeout(400);
  }
  const clean = await page.evaluate(() => window.ftDebug.designer().tint);
  if (clean) fail(`${label}: wipe left ${clean} painted cells`);
  await page.click('#dzBrushDown');
  await page.waitForTimeout(200);
  if ((await page.evaluate(() => window.ftDebug.designer().brushSlot)) !== null)
    fail(`${label}: the brush stayed armed after being put down`);
  await page.click('#dzTabParts');
  await page.waitForTimeout(200);
}
 ...
 * over the sliders somebody opened it to use. And the elaboration is reachable
 * by TAP, because a phone has no hover and a tooltip nothing can open is a
 * tooltip that does not exist.
 */
async function checkCopy(page, label) {
  const COPY_MAX = 50;
  const prose = await page.evaluate(max => {
    ...
    // structure it is asking for. A RUN OF PROSE is what the rule limits, so
    // measure the paragraphs and not the container.
    const BLOCK = 'div,p,dl,ol,ul,table,section,figure,dt,dd,li,tr,td,th,thead,tbody';
    const measure = (el) => {
    ...
    for (const el of document.querySelectorAll('#designer .dznote, #designer .hint, #designer .dzmore')) {
  ...
  for (const b of prose) fail(`${label}: ${b.n} characters of copy visible by default: "${b.t}..."`);
  if (!prose.length) ok(`${label}: no block of copy over ${COPY_MAX} characters is visible by default`);

  const why = await page.$$('#dzPaneArmour .dzwhy');
  if (!why.length) fail(`${label}: no way to reach the elaboration that used to be on screen`);
  else {
    ...
    if (before) fail(`${label}: a footnote is open before anything asked for it`);
    else if (!opened) fail(`${label}: the ? opened nothing`);
    else if (closed) fail(`${label}: the ? does not close again`);
    else ok(`${label}: ${why.length} headings fold their elaboration away, and it opens on a `
