...
function renderHelp(): void {
  ...
  // for: the toggle below owns the opening and this only has to hang the
  // text on the heading the reference sits under.
  const head = $('helpHead');
  ...
  why.title = 'The outline is where the ship can actually finish its turn on '
    ...
    + 'it, and two fingers pan and pinch without ever touching a plan. Reading '
    + 'a ship: the chips list what is offline beside each hull, and zoomed in, '
    + 'Ship data labels its systems where they are.';
...
async function showRoute(r: route.Route): Promise<void> {
  ...
  switch (r.kind) {
    ...
    case 'architect': {
      // No class named: the first of them. A picker screen with one job would
      // be a third place that knows the class list, and the yard's chips
      // already are one.
