...
/** The `?` that folds a heading's elaboration away; `main.ts` owns the toggle. */
const why = (text: string): HTMLButtonElement => {
...
export class Architect {
  ...
  render(): void {
    ...
    // A label, not a paragraph: the why is behind the `?` on the heading.
  ...
  #renderEdit(): void {
    ...
    if (!sel) {
      ...
      p.className = 'dznote';
      p.textContent = 'Tap a station above to edit it.';
      host.appendChild(p);
    ...
    head.appendChild(why('Where this station sits in the hull\u2019s lattice, in cells '
      + 'from the corner: beam is across the ship, depth is up it, length is along '
      + 'it. Move it and the hull is re-rasterised under you, so the part that '
      + 'seats here moves with it.'));
    const who = document.createElement('p');
    who.className = 'dznote';
    who.innerHTML = `<b>${esc(sel.label)}</b> &middot; ${esc(sel.kind)}, in cells`;
    ...
    kindHead.appendChild(why('A station\u2019s kind is what the yard will offer to '
      + 'bolt into it: a drive plate offers bells, a gun ring offers a barbette. '
      + 'Changing it changes the shopping list, not the geometry.'));
    host.appendChild(kindHead);
    ...
    nameHead.appendChild(why('What the shipyard calls this station in its own list, '
      + 'so "drive, port lower" reads as a place on a ship rather than as a row '
      + 'number.'));
    host.appendChild(nameHead);
