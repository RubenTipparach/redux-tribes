 ...
 */
import {
  NX, NY, NZ, CELLS, paintFor, DECAL_STRIDE, decalKey,
  FRAMES, SECTIONS, SOCKET_KINDS, frameFor, moduleById,
  socketsOf, stockFor, stockFrameFor,
  type Design, type FrameDef, type LaneHint, type Placement, type Socket, type SocketKind,
} from './design.js';
 ...
 *
 * A frame's sockets are CELLS on the one lattice every hull shares, 32 x 32 x
 * 64, and a cell index means nothing on any other lattice: a file written for
 * a different one would hold coordinates all in range and all in the wrong
 * place, which is exactly the shape change a reader cannot otherwise detect.
 * So the lattice is part of what the version names, and a file from a build
 * with another lattice is refused by name rather than half loaded.
 */
...
export function toFile(f: FrameDef): FrameFile {
  return {
    ...
    note: f.note,
    // Sorted by id and written in ONE key order, so two exports of one frame
    // are the same bytes: a file that reorders itself is a file every diff is
    // noise in, and `edited` compares frames as files, so a socket whose keys
    // merely came out in another order would read as a change.
    sockets: [...f.sockets].sort((a, b) => (a.id < b.id ? -1 : a.id > b.id ? 1 : 0))
      .map(canonSocket),
  };
}

/** A socket with its keys in the order the file writes them. */
const canonSocket = (s: Socket): Socket => ({
  id: s.id,
  kind: s.kind,
...
function readSocket(v: unknown, n: number): { sock?: Socket; why?: string } {
  ...
  // A berth's LANE hint rides with it, or a round trip through a file would
  // read as an edit and move every berth to its conservative seat. It is a
  // hint `socketsOf` re-seats against the rings, so a bad one costs a seat
  // rather than a lattice write, and it is carried only when it is an object.
  const lane = o['lane'];
  const sock: Socket = {
    id,
    ...
    ...(lane && typeof lane === 'object' ? { lane: lane as LaneHint } : {}),
  };
  return { sock };
}
...
export function designFromJson(text: string): { design?: Design; name?: string; why?: string } {
  ...
  // Cell lists are bounded against the one lattice every hull is drawn on: a
  // file is a list of array subscripts somebody else chose.
  ...
  // is what names the texture and an unknown one draws nothing; the eraser is
  // a kind and comes through.
  const decal = src['decal'];
  if (Array.isArray(decal)) {
    out.decal = [...new Set(decal.filter(n => typeof n === 'number'
      && Number.isInteger(n) && n >= 0 && n < CELLS * DECAL_STRIDE
      && decalKey(n % DECAL_STRIDE) !== undefined))].sort((a, b) => a - b);
  }
  ...
  // The per band finishes, the same sparse shape as the per slot ones below.
  const bands = src['bandFinish'];
  if (Array.isArray(bands)) {
    out.bandFinish = bands.slice(0, 8).map(v => (typeof v === 'string' ? v : null));
  }
