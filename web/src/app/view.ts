...
/**
 * Every texture this file loads, from the SITE ROOT.
 *
 * A leading slash rather than `./`, for the reason the wasm is loaded that
 * way and spelled out where it is: this page is served from `/play/<id>` as
 * well as from `/`, and `./ember.png` there is `/play/ember.png`, which the
 * shell route answers with the index page. A texture handed HTML does not
 * throw. The load quietly fails, the material keeps a normal map with no
 * pixels in it, and the hull draws exactly as it did before the finish was
 * added, so the change looks like one that was never made. That is how the
 * ember and all nine finishes shipped dead. `ftDebug.surfaces()` reports
 * whether each one has pixels now, and the playthrough fails if one has not.
 */
const TEX = (name: string): string => `/${name}`;
...
function ember(): THREE.Texture {
  if (!emberMap) {
    emberMap = new THREE.TextureLoader().load(TEX('ember.png'));
...
function finishMap(key: string): THREE.Texture | null {
  ...
  if (had) return had;
  const t = new THREE.TextureLoader().load(TEX(`surf/armour_${key}_n.png`));
...
export class View {
  ...
  constructor(canvas: HTMLCanvasElement, match: Match, sim: Sim) {
    ...
    new THREE.TextureLoader().load(TEX('surf/env.png'), tex => {
  ...
  focusOn(ship: ShipState): void {
    ...
    this.#focus.copy(this.#livePos(ship));
  ...
  closeUpOn(ship: ShipState): boolean {
    ...
    return this.#focus.distanceTo(this.#livePos(ship)) <= span * INSPECT_OFF;
  }
