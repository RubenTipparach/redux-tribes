...
export class View {
  ...
  //
  // Two of everything: a GOAL that input writes and a value the camera is
  // drawn from, eased toward it. A camera that snapped told you where it had
  // arrived and nothing about how it got there, which on a fleet spread over
  // hundreds of units is a cut rather than a move.
  //
  // The lock is the exception and deliberately so. Following a hull, the goal
  // moves every tick; easing toward a target that moves every tick is a camera
  // that never arrives and always lags, which reads as jitter rather than as
  // smoothness. So a follow EASES IN once and then locks: `#locked` goes true
  // when the ease has closed the gap, and from there the focus is copied
  // exactly.
  ...
  #refreshWound(id: number, c: Carved, tick: number): void {
    ...
    if (c.woundFor !== c.cells.size) {
      ...
      if (!keepSkin || !keepGlow) {
        // A plate cell that survived a hit is still PLATE, so it keeps wearing
        // the hull's finish. Losing that was the defect: the wound's surfaces
        // carried no UVs at all, so the normal map had nothing to sample and a
        // shot took the plating off a whole region, which is exactly what a
      ...
      const s = this.#ships.find(x => x.id === id);
      if (s && c.woundSkin) {
        tintHull(c.woundSkin.material as THREE.MeshStandardMaterial,
  ...
  #drawDebris(chunks: ReadonlyArray<{ at: THREE.Vector3; dir: THREE.Vector3; age: number; hex: number }>): void {
    if (!this.#debris) {
      this.#debris = new THREE.InstancedMesh(
        ...
        // Standard, so a chunk in the air is lit by the same reflection as the
        // hull it came off. No normal map: a chunk is a few pixels across and
        // one would be a texture fetch for nothing.
        new THREE.MeshStandardMaterial({
          transparent: true, metalness: 0.4, roughness: 0.65, dithering: true,
        }),
        DEBRIS_MAX);
  ...
  orbit(dx: number, dy: number): void {
    ...
    this.#goalPitch = Math.max(-1.45, Math.min(1.45, this.#goalPitch + dy * 0.005));
  }
  ...
  pan(dx: number, dy: number): void {
    ...
    this.#breakFollow();
    this.#goalFocus.addScaledVector(right, -dx * scale);
    this.#goalFocus.addScaledVector(up, dy * scale);
  }
  ...
  zoom(factor: number): void {
    ...
    this.#goalDist = Math.max(9, Math.min(900, this.#goalDist * factor));
  }

  centreOn(p: Vec3): void {
    // A deliberate move to a POINT, which is not a ship, so the lock goes.
    this.#breakFollow();
    this.#goalFocus.set(p.x, p.y, p.z);
  }
  ...
  focusOn(ship: ShipState): void {
    ...
    this.#goalFocus.copy(this.#livePos(ship));
    this.#goalDist = Math.max(9, Math.min(this.#goalDist, span * (INSPECT_SPANS - 2)));
    this.#follow = ship.id;
    // Not locked YET. The camera flies to the hull and locks when it gets
    // there; locking first would teleport, which is the thing this replaces.
    this.#locked = false;
  }

  /**
   * Where a hull IS, right now, on screen.
   *
   * `ShipState.pos` is the pose the turn STARTED from and is only refreshed
   * when the console reads the world back; `setPoses` moves the meshes every
   * tick of a playback without touching it. Anything drawn ON a hull, or
   * pointed at one, has to ask the mesh instead, or it tracks a ship to where
   * it used to be and lets it fly away. One place answers it, so the camera,
   * the close up test and the volume labels cannot drift apart.
   */
  poseOf(id: number): { pos: THREE.Vector3; quat: THREE.Quaternion } | null {
    const mesh = this.#hulls.get(id);
    if (!mesh) return null;
    return { pos: mesh.position, quat: mesh.quaternion };
  }

  /** Where a ship is, from the mesh when there is one and the state when there
   *  is not: a hull that has never been built has nowhere else to be. */
  #livePos(ship: ShipState): THREE.Vector3 {
    return this.#hulls.get(ship.id)?.position ?? v(ship.pos);
  }

  /**
   * Put the focus back on the hull it is locked to, now that the hull has
   * moved.
   *
   * Called from the two places a hull's pose changes, rather than only from
   * the frame loop, because the console judges things BETWEEN frames. Ending a
   * turn moves every mesh to the pose the turn ended at and then asks whether
   * the camera is still close enough to offer the ship data overlay: with the
   * focus a frame behind, a hull that had flown forty units answered no and
   * the labels came down at the very moment the turn a player wanted to watch
   * began. A lock that is only applied when something draws is not a lock.
   */
  #trackFollow(): void {
    if (this.#follow < 0) return;
    const at = this.#hulls.get(this.#follow)?.position;
    if (!at) return;
    this.#goalFocus.copy(at);
    // Once the camera has arrived it stops easing and simply IS on the hull.
    // Easing a target that moves every tick would lag it by a frame forever,
    // which is jitter with a smooth name.
    if (this.#locked) this.#focus.copy(at);
  }

  /** Stop following, because the player has taken the camera somewhere. */
  #breakFollow(): void { this.#follow = -1; this.#locked = false; }
  ...
  fit(): void {
    ...
    this.#breakFollow();
    box.getCenter(this.#goalFocus);
    const size = box.getSize(new THREE.Vector3()).length();
    this.#goalDist = Math.max(60, size * 1.4 + 40);
  }
  ...
  /**
   * How fast the camera closes on its goal, per second.
   *
   * `1 - exp(-k * dt)` rather than a fixed fraction per frame, so the ease
   * takes the same WALL TIME on a machine drawing 20 frames a second as on one
  ...
  #applyCamera(dt = 0.016): void {
    ...
    if (this.#follow >= 0) {
      // The MESH, not the state: through a playback the state stands still at
      // the pose the turn began from and only the mesh moves, so a camera
      // locked to the state watches the ship leave.
      this.#trackFollow();
    }
    // Ease toward the goal, except on a hull the camera has already reached.
    const t = this.#ease(dt);
    // The ORBIT always eases, locked or not: turning round a ship you are
    // following is still a camera move and still wants to be smooth. Only the
    // FOCUS is exempt, and only once the camera has arrived on the hull.
    // Yaw takes the short way round, or a drag past the seam unwinds the whole
    // circle the other way.
    this.#yaw += wrapPi(this.#goalYaw - this.#yaw) * t;
    this.#pitch += (this.#goalPitch - this.#pitch) * t;
    if (!this.#locked) this.#focus.lerp(this.#goalFocus, t);
    // Distance eases in LOG space, because zoom is multiplicative: a step from
    // 900 to 800 and one from 20 to 18 are the same gesture, and a linear
    // lerp makes the first crawl and the second snap.
    this.#dist *= Math.pow(this.#goalDist / this.#dist, t);
    // Arrived. From here a followed hull is copied exactly rather than chased.
    if (this.#follow >= 0 && !this.#locked
        && this.#focus.distanceTo(this.#goalFocus) < LOCK_EPS
        && Math.abs(this.#dist - this.#goalDist) < LOCK_EPS) {
  ...
  closeUpOn(ship: ShipState): boolean {
    const span = Math.max(ship.radius, 2);
    // The GOAL, not the eased value. A camera on its way in is a camera that
    // has been sent, and testing where it happens to be this frame would flash
    // the overlay off for the fifth of a second the move takes.
    if (this.#goalDist > span * INSPECT_SPANS) return false;
    return this.#goalFocus.distanceTo(this.#livePos(ship)) <= span * INSPECT_OFF;
  }
  ...
  pickSub(clientX: number, clientY: number, volumes: ReadonlyArray<SubState>): number {
    ...
    const live = this.poseOf(first.ship);
    if (!ship || !live) return -1;
    const rect = this.#canvas.getBoundingClientRect();
    const ray = new THREE.Raycaster();
    ...
    // The volumes come from a world read that happens once a turn; the hull
    // MOVES every tick of a playback. So the offset is taken from the pair
    // that were read together, and it is placed by the pose on screen.
    const inv = live.quat.clone().invert();
    const o = ray.ray.origin.clone().sub(live.pos).applyQuaternion(inv);
    const local = new THREE.Ray(o, ray.ray.direction.clone().applyQuaternion(inv));
    const box = new THREE.Box3();
  ...
  /**
   * A volume's offset in its ship's own frame.
   *
   * `SubState` carries a WORLD point, read when the console last read the
   * world, and a hull moves every tick of a playback without that read
   * happening again. Both halves of the pair are equally stale, so their
   * difference is still the right offset; it is placed by the pose on screen
   * rather than by the one it was read at.
   */
  #subLocal(sub: SubState, ship: ShipState): THREE.Vector3 {
    return new THREE.Vector3(
      sub.pos.x - ship.pos.x, sub.pos.y - ship.pos.y, sub.pos.z - ship.pos.z)
      .applyQuaternion(new THREE.Quaternion(
        ship.quat.x, ship.quat.y, ship.quat.z, ship.quat.w).invert());
  }

  /** Where a volume is in the world right now, on the hull as it is drawn. */
  subWorld(sub: SubState): THREE.Vector3 {
    const ship = this.#ships.find(x => x.id === sub.ship);
    const live = this.poseOf(sub.ship);
    if (!ship || !live) return v(sub.pos);
    return this.#subLocal(sub, ship).applyQuaternion(live.quat).add(live.pos);
  }

  /** Where a volume lands on screen, so its label can sit on it while the
   *  hull it belongs to is flying. */
  subScreen(sub: SubState): { x: number; y: number } {
    return this.screenOf(this.subWorld(sub));
  }
  ...
  pickShip(clientX: number, clientY: number): number {
    ...
    for (const s of this.#ships) {
      ...
      // place a player could click to focus on it stayed behind while the ship
      // flew off, and clicking the ship they could SEE did nothing. Same
      // defect as the camera lock, in the same place, one field over.
      const sphere = new THREE.Sphere(this.#livePos(s), Math.max(s.radius * 1.8, 4));
      const hit = new THREE.Vector3();
  ...
  #buildHull(s: ShipState): THREE.Mesh {
    ...
    // three maps and the hull has one material; there are a few dozen quads
    // against a thousand, so it is one small draw per decal kind.
    for (const w of hull.windows) {
      const wm = windowMaterial(w.key);
      if (wm) mesh.add(new THREE.Mesh(w.geo, wm));
    }
  ...
  cameraState(): {
    ...
    return {
      ...
      follow: this.#follow,
      focus: { x: +this.#focus.x.toFixed(3), y: +this.#focus.y.toFixed(3), z: +this.#focus.z.toFixed(3) },
      /** Whether the ease has arrived. Locked means the focus IS the hull,
       *  copied rather than chased, which is what stops a follow jittering. */
      locked: this.#locked,
      goalDist: +this.#goalDist.toFixed(2),
    };
  }

  /**
   * What the hulls are actually MADE of on screen: the material, its two PBR
   * numbers, and whether the finish is bound AND has pixels yet.
   *
   * A texture that fails to load is a texture that draws as flat, silently,
   * and a material that was quietly swapped back to Lambert draws the same
   * way. Neither shows up in a suite that reads state, and both look like a
   * change that was never made. So the map says what it is using.
   */
  surfaces(): Array<{
    ...
    wounds: Array<{ what: string; uv: boolean; loaded: boolean }>;
  }> {
    const out = [];
    for (const [id, mesh] of this.#hulls) {
      ...
      for (const [what, w] of [['plate', c?.woundSkin], ['inner', c?.woundInner]] as const) {
        if (!w) continue;
        const wm = w.material as THREE.MeshStandardMaterial;
        const wi = wm.normalMap?.image as { width?: number } | undefined;
        wounds.push({
          what,
          uv: !!w.geometry.getAttribute('uv'),
          loaded: !!wi?.width,
        });
      }
      out.push({
        ...
        rough: +(m.roughness ?? -1).toFixed(3),
        finish: n ? (img?.src ?? 'bound').split('/').slice(-1)[0] as string : 'none',
        loaded: !!img?.width,
        repeat: n ? +n.repeat.x.toFixed(4) : 0,
  ...
  render(): void {
    ...
    // `dt` because the camera eases now, which main taught it while this
    // branch was out.
    this.#applyCamera(dt);
