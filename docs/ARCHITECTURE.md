...
- **Move modes are re-expressed, not dropped.** `MOVE_AND_TURN` auto-faces the course it was given, so a ship arrives looking the way it went (DESIGN 3.2). It pointed the nose where thrust was needed until the heading was first asserted, which sounds equivalent and is not: the wanted velocity falls to zero on arrival, so the nose swung fully retrograde at the end of every move. `FULL_STOP` still aims at the thrust, since a hull braking to a dead stop should come round and brake on its main drive. `TURN_SLIDE` holds a commanded heading and hands the course to the RCS, which is a far smaller envelope bought in exchange for keeping guns on a bearing. `FULL_SPEED` and `FULL_STOP` are committed: the order chooses no destination, so their envelope is a single point, and the planner says so rather than drawing an empty volume.
...
- **One deploy target, not two.** The same Fly machine serves the client and the API. A static bundle of a few hundred kilobytes does not justify a second pipeline, and hosting both together means the page and the API it talks to are always the same build, which separate deploys cannot promise. It also keeps two things out of the critical path that turned out to bite: GitHub Pages enablement, and artifact storage quota, which is account wide and not something this repository controls. The image is multi stage: Rust compiles the core to wasm, Node bundles the client against it, and the runtime stage carries only the server plus the built page. Note that `.wasm` must be served as `application/wasm` or `WebAssembly.instantiateStreaming` refuses it at runtime, so the type is set explicitly rather than trusted to a lookup table.
