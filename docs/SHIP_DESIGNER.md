...
Two things are deliberately NOT scanned. An enclosed missile bay: it sits inside the hull by construction and fires through its own doors, so line of sight from its socket is blocked in every direction at once, and scanning it would have taken the launcher off two of the five stock frigates for a reason nobody asked for. Only mounts on proud sockets, trunnions and gun rings, are scanned. And a class ship in a match carries no mask, because it carries no design: the rasteriser is still on the client, so the core has no voxels for a hull nobody built. That closes when the rasteriser moves into the core.

**The scan is debounced, and this is the difference between affordable and unusable.** It is about 2 to 7 ms a mount, which is a frame; the armour pencil fires an edit per cell dragged through, so scanning inline would have queued one per cell and made the pencil stutter under its own feedback, exactly as the reach envelope did. It runs a second and a half after the last edit, whatever the edit was. The shipyard harness pins both halves: a nine cell stroke costs zero scans while the finger is down and exactly one after it lifts.

**The bin geometry comes from the core, not from `Math.sin`.** The client scans; which directions it must scan is not a measurement, it is the mask's own geometry, and the resolver reads the mask back with `atan2` on fixed polynomials. A client that built those angles out of its platform's `sin` would set a different bit on a boundary from the client across the table and desync over a shot one seat allowed. So `ft_arc_dirs` hands out all 2048 cell centres and `ft_arc_bit` bins a direction back, and `design.ts` still calls no transcendental at all. The mask is hashed with the rest of the state, so two seats that scanned differently part on the turn it happens rather than several turns later.

**A word cannot ride in an f32 slot.** The scratch buffer between the client and the core is `f32`, which carries whole numbers exactly only to 2^24, so a mask word with its top bit set would arrive rounded and hand the mount an arc nobody scanned. Each word crosses as two halves, low then high. The web suite sets one half of the word the target sits in and then the other, and fails if either the whole or the wrong half blocks the shot.

The designer draws the arcs and works nothing out from them. Whether a shot is legal in a match is `ft_can_bear`'s answer now, asked by the console before it offers a mount and by the resolver before it fires: one predicate, `Sim::bears`, so the greyed out mount and the dropped shot cannot come from two different opinions.
...
**The preview is two buttons over the model**, `Arcs` and `Target`, beside the plate toggle. Arcs draws a filled wedge in the horizontal plane and an unfilled one in the vertical, per turret, coloured by purpose, plus the scanned shadow: one red patch per blocked cell on the shell that mount would otherwise cover. The shadow is bolted to the hull rather than to the barrel, because that is where the blockage is, and it does not swing when the turret does. Target puts a marker in orbit around the hull for the turrets to chase, with a sight line from each one that bears. They are independent switches: watch the wedges without the target, or the target without the wedges.
...
**FACTION, eight swatches, and the one you pick is the one you get.** It used to spread all eight over the hull by position, as plating panels, a dark underside, a dorsal spine, a waist stripe, a nose flash and a transom band, with the picked swatch as the primary and the other seven roles holding. It made a handsome ship and it took the decision away: a player who picked a colour got a scheme built around it rather than the colour they picked. The armour is now the swatch that was chosen and nothing else, and the eight are eight things to choose between rather than eight roles to be assigned. The harness checks the pick reaches the hull and that picking another changes it.
...
| armour colour | the one picked | the one picked | the one picked | the one picked | the one picked |
...
**Thickness cuts the hull into slabs, it does not stack overlapping slices.** The first version read the slider as "and the next five slices as well", so slice 32 and slice 33 both wrote z 33 and the number under the slider stopped meaning a place. Thickness `t` now tiles the 64 cells into `ceil(64 / t)` slabs and the slider walks the slabs: at 1 there are 64 of them and at 6 there are 11, each 6 deep and none sharing a cell with its neighbour. A stroke paints every z in the slab it is standing in, which is how a run becomes a rib rather than a line. Changing the thickness keeps the cursor on the z it was already on rather than on the index it was already on, because 32 out of 64 and 32 out of 11 are not the same place: the slab you land on is the one holding z 32.

**And the slab is drawn on the model.** A flat grid on the right says what you are drawing and nothing at all about where. An amber box round the part of the hull the slab cuts, with its two cut planes filled at 6 percent, grows and shrinks with the thickness: 0.109 units at 1 and 0.656 at 6. It is drawn with `depthTest` off so it reads through the plate, and only while the Armour tab is open, because a box round a slab you cannot draw on is decoration over the thing you are trying to look at.

**Onion skin.** The slabs either side are ghosted, cyan aft and amber forward, dimmer the further out, up to four.

**Symmetry is a toggle, and only x and y.** Off by default: a hangar mouth down one side is a thing people want, and a tool that silently doubles every stroke cannot draw one. On, one tap lays two cells, and with both axes on, four. The columns are deduped before they are painted, because a cell on a mirror plane is its own mirror and painting it twice with the cut brush would undo it. There is no z mirror: the bow and the stern of a ship are not the same shape, and rolling a design about its long axis is not a thing anyone asked a hull to do.
